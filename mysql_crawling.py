...
#mysql db 연결
#HOST URL, user, password, db는 상황에 맞게 입력
connect = pymysql.connect(host='HOST URL',
                          user='root',
                          password='****',
                          db ='****',
                          charset ='utf8')
...
#1~1200까지 100개 단위로 크롤링
for i in range(12):

    print('------------INDEX %3d TO %d--------------'%(start_index, end_index))
    print("-----------------PROCESSING-----------------")
    #본 코드는 식품안전부 레시피 db를 크롤링해오는 코드
    #KEY는 본인이 발급받은 인증키 삽입
    url = "http://openapi.foodsafetykorea.go.kr/api/KEY/COOKRCP01/xml/"+str(start_index)+ "/" + str(end_index)
    #url 접속 요청 객체 생성
